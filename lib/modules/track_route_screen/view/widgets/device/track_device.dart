...
class _TrackDeviceViewState extends State<TrackDeviceView>
        ...
        return PopScope(
          ...
          child: Stack(
            ...
            children: [
              ...
              if (!controller.expandInfo.value) ...[
                Positioned(
                    bottom: MediaQuery.of(context).size.height < 670
                        ? MediaQuery.of(context).size.height * ((205 / 812) - 0.03) +
                            24
                        : MediaQuery.of(context).size.height * ((205 / 812) - 0.067) +
                            24,
                    child: SpeedometerWidget(
                        color: (controller.deviceDetail.value?.trackingData?.currentSpeed ?? 0) == 0
                            ...
                            : (controller.deviceDetail.value?.maxSpeed != null
                                ...
                                : AppColors.selextedindexcolor),
                        speed:
                            ((controller.deviceDetail.value?.trackingData?.currentSpeed ?? 0)
                                    .toStringAsFixed(0) ??
                                "N/A"),
                        distance: controller.deviceDetail.value?.trackingData?.dailyDistance ?? 0)),
                Positioned(
                  bottom: MediaQuery.of(context).size.height < 670
                      ? (MediaQuery.of(context).size.height * ((215 / 812)) +
                          16)
    ...
    return [
      ...
      if (displayParameters?.door == true)
        _buildVehicleItem(
            context,
            'Door',
            controller.deviceDetail.value?.trackingData?.door,
            'assets/images/svg/new_door.svg'),
      if (displayParameters?.relay == true)
        _buildVehicleItem(
            context,
            'Relay',
            controller.deviceDetail.value?.immobiliser == null
                ? null
  ...
  Widget _infoWidget(BuildContext context, bool isNotExpired, bool isActive) {
    ...
    return Container(
      height: MediaQuery.of(context).size.height < 670
          ? MediaQuery.of(context).size.height * (0.32 - 0.03)
          : MediaQuery.of(context).size.height * (0.32 - 0.063),
              ...
              InkWell(
                onTap: () {
                  Clipboard.setData(ClipboardData(
                      text: controller.deviceDetail.value?.imei ?? ""));
  ...
  Widget _fullInfoWidget(
      BuildContext context, bool isNotExpired, bool isActive) {
    String date = 'Update unavailable';
    String time = "";
    if (controller
            .deviceDetail.value?.trackingData?.lastUpdateTime?.isNotEmpty ??
    ...
    return Column(
      children: [
        InkWell(
            ...
            child: SvgPicture.asset(
              "assets/images/svg/down_arrow_button.svg",
              width: 24,
              height: 24,
            )),
        Container(
